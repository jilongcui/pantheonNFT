...
contract DogeFoodSoloPool is Ownable, ERC721Holder {
    ...
    function pendingReward(address _user, uint256 idx)
        external
        view
        returns (uint256)
    {
    ...
    function withdraw(uint256 _pid, uint256 _idx) public {
        ...
        updateReward(0);
        if (miner.power.mul(accChaPerShare).div(1e12) > miner.rewardDebt) {
            pending = miner.power.mul(accChaPerShare).div(1e12).sub(
                miner.rewardDebt
            );
            safeChaTransfer(msg.sender, pending);
        }
